...
let rec cStmt stmt (varEnv : VarEnv) (funEnv : FunEnv) (labellist : LabelEnv)(C : instr list) : instr list = 
    ...
    | Dowhile(body, e) ->
        ...
        Label labelbegin :: cStmt body varEnv funEnv labellist C1 //��ִ��body
    | Expr e -> 
    ...
    | For(dec, e, opera,body) ->
        let labelend   = newLabel()                       //����label
        let labelbegin = newLabel()                       //��ʼlabel 
        let labelope   = newLabel()                       //���� for(,,opera) ��label
        ...
        cExpr dec varEnv funEnv labellist (addINCSP -1 (addJump jumptest  (Label labelbegin :: C4) ) ) //dec Label: body  opera  testjumpToBegin ָ���˳��
    | Try(stmt,catchs)  ->
